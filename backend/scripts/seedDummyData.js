...
async function seedDB() {
  try {
    const adminUser = await userRepository.findByEmail('admin@ceyloncompass.dummy');
    if (!adminUser) {
      console.error('No dummy admin user found. Run "npm run seed:admin" first.');
      process.exit(1);
    }

    await locationRepository.create('Sri Lanka', [
      'Colombo', 'Kandy', 'Galle', 'Ella', 'Matara', 'Nuwara Eliya'
    ]);
